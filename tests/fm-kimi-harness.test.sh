...
# Retained on this fork after upstream removed it in the source-assertion
# cleanup. It is not a source assertion - it greps tracked files for a leaked
# developer path - and it earns its place here: it caught an absolute Windows
# path that a generated ledger reason had captured verbatim, in this branch,
# after CI had already gone green once without it.
#
# The pattern is a REGEX whose first character is a bracket expression, not a
# literal leading slash, for two reasons at once:
#
#   * it cannot match this line, so the check does not flag its own source;
