...
#
# The four leading entries are this branch's additions, required by the
# transitive-source-closure guard in tests/fm-lint.test.sh: fm-platform-lib.sh
# is new here, and the other three were already-unsatisfied sources the guard
# surfaced. fm-x-lib.sh was a fifth until upstream added it independently.
OLD_BIN_UNCHANGED_SIBLINGS="fm-platform-lib.sh fm-busy-lib.sh fm-startup-memory-budget-lib.sh fm-check-lib.sh fm-push-transition-lib.sh fm-transition-lib.sh fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh fm-public-followup-lib.sh fm-x-lib.sh"
