...
# Retained on this fork after upstream removed the source-graph test family.
# This one is not a restatement of the source graph: it checks that a FIXTURE
# installing a production script also installs what that script sources. Five
# separate fixture breakages were caught by it, each of which had produced a
# confusing runtime failure rather than a clear one.
#
